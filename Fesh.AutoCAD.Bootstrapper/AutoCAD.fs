...
module AutoCAD =
    ...
    // https://help.autodesk.com/view/RVT/2025/ENU/?guid=AutoCAD_API_AutoCAD_API_Developers_Guide_Introduction_Add_In_Integration_Add_in_Registration_html
    let getXml (year:string) (dllPath:string) = $"""<?xml version="1.0" encoding="utf-8"?>
<AutoCadAddIns>
  <AddIn Type="Application">
    <Name>Fesh | F# Editor and Scripting Host for AutoCAD {year}</Name>
    <Assembly>{dllPath}</Assembly>
    <FullClassName>Fesh.AutoCAD.FeshAddin</FullClassName>
    <AddInId>5B83A504-FF2D-4BAE-97CB-0DEB1046A5C2</AddInId>
    <VendorId>Goswin Rothenthal</VendorId>
    <VendorDescription>Fesh | F# Editor and Scripting Host for AutoCAD {year}</VendorDescription>
  </AddIn>
</AutoCadAddIns>
"""

    let yearsNet48 = [| 2021 .. 2024 |]
    ...
    let searchAddinFolder =
         ...
         IO.Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.ProgramFiles), "Autodesk", "ApplicationPlugins")


    let findAddinFolders() =
        if not (IO.Directory.Exists searchAddinFolder) then
            eprintfn $"AutoCAD Addin root directory not found:\r\n{searchAddinFolder}\r\nis AutoCAD installed?"
            None
        else
            ...
            if ok.Length = 0 then
                if notOk.Length = 0 then
                    eprintfn $"No AutoCAD version subfolder found in {searchAddinFolder}. Is AutoCAD installed?"
                elif isNetFramework then
                    printfn $"These AutoCAD addin folders for AutoCAD 2025 or later exist:"
                    for no in notOk do printfn $"    {no}"
                    eprintfn $"Please use the installer based on .NET 8 for AutoCAD 2025 and later versions:"
                    eprintfn $"https://github.com/goswinr/Fesh.AutoCAD/releases"
                else
                    printfn $"These AutoCAD addin folders for AutoCAD 2024 and earlier exist:"
                    for no in notOk do eprintfn $"    {no}"
                    eprintfn $"Please use the installer based on .NET FRamework 4.8 for AutoCAD 2024 and earlier versions:"
                    eprintfn $"https://github.com/goswinr/Fesh.AutoCAD/releases"
                None
            else
                for dir in notOk do
                    ...
                    if IO.File.Exists dllPath then
                        ...
                        eprintfn $"It looks like you also have AutoCAD {year} installed."
                        if isNetFramework then
                            eprintfn $"To also use Fesh there, please install the .NET 8 based version of Fesh for AutoCAD 2025 and later."
                        else
                            eprintfn $"To also use Fesh there, please install the .NET 4.8 based version of Fesh for AutoCAD {yearsNet48[0]} to 2024."
                Some ok
    ...
    let register(log:AvalonLog.AvalonLog) =
        ...
        | Some dirs ->
            for dir in dirs do
                ...
                let dllPath = IO.Path.Combine(exeDir, "Fesh.AutoCAD.dll")
            ...
            log.printfBrush  Brushes.DarkGreen $"Fesh was installed and registered with AutoCAD."
    ...
    let unregister() =
        for year in 2018 .. 2030 do
            let addinFile = IO.Path.Combine(@"C:\ProgramData\Autodesk\AutoCAD\Addins", $"{year}", "Fesh.addin")
